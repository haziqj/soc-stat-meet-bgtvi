...
\begin{frame}{Zero-forcing vs Zero-avoiding (cont.)}
  ...
  \only<1>{\includegraphics[scale=0.65]{figure/zero_force_1}}
  \only<2>{\includegraphics[scale=0.65]{figure/zero_force_2}}
  \only<3>{\includegraphics[scale=0.65]{figure/zero_force_3}}
  \only<4>{\includegraphics[scale=0.65]{figure/zero_force_4}}      
\end{figure}
...
\begin{frame}{Distortion of higher order moments}
  ...
  \begin{figure}[t]
    ...
    \only<1>{\includegraphics[scale=0.7]{figure/distorted_mean_1}}
...
\begin{frame}{Quality of approximation}
%  \blfootnote{\fullcite{gunawardana2005convergence}}
  ...
  \begin{tikzpicture}[xscale=1.4]
 	...
 	\node[right,text width=2cm,align=left] at (6,0) {\color{gray}\small Slow and accurate};
  	...
  	\node[align=center,below] at (1.3,-1.8) {Laplace};  	  
